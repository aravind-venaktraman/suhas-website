...
import { Menu, X, Instagram, Twitter, Youtube, Music, ArrowRight, ShoppingBag, ExternalLink, Play, Disc, Heart, DollarSign, Mail, Headphones } from 'lucide-react';
...
const LoadingScreen = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      if (p >= 100) {
        ...
        setTimeout(() => { setPhase('done'); onCompleteRef.current(); }, 1000);
      } else {
  ...
  return (
    <div
      className={`fixed inset-0 z-[300] bg-black flex flex-col items-center justify-center transition-opacity duration-600 ${phase === 'revealing' ? 'opacity-0' : ''}`}
...
const AbstractPiano = ({ isExpanded, onPlayNote }) => {
  ...
  const playNote = async (index, velocity = 0.8, release = true) => {
    ...
    try {
      ...
      if (arpeggiator) {
        ...
        pattern.forEach((note, i) => setTimeout(() => samplerRef.current.triggerAttackRelease(note, '16n', undefined, velocity), i * 100));
      } else {
        samplerRef.current.triggerAttackRelease(noteName, '2n', undefined, velocity);
      }
      if (release) setTimeout(() => setActiveNotes(prev => { const s = new Set(prev); s.delete(index); return s; }), 200);
  ...
  const playRecording = () => {
    ...
    setIsPlaying(true);
    const startTime = Tone.now(), firstNoteTime = recordedNotes[0].time;
    recordedNotes.forEach(({ note, time, velocity }) => samplerRef.current.triggerAttackRelease(note, '2n', startTime + (time - firstNoteTime), velocity));
    ...
    <div className="w-full space-y-6">
        ...
        {[...Array(numKeys)].map((_, i) => {
          ...
          return (
            <button
              ...
              onTouchStart={(e) => { e.preventDefault(); playNote(i, 0.5 + (e.touches[0].clientY / window.innerHeight) * 0.5); }}
              className={`transition-all duration-100 ease-out cursor-pointer active:scale-95 relative ${
                isBlackKey
                  ? `z-10 rounded-b-lg border shadow-lg shadow-black/80 ${isActive ? 'bg-cyan-400 border-cyan-300' : 'bg-zinc-950 border-zinc-800 active:bg-zinc-800'} ${isExpanded ? 'w-6 h-24 -mx-3' : 'w-5 h-14 -mx-2.5'}`
                  : `rounded-b-lg border-t-2 border-x border-b hover:from-white/35 hover:via-white/25 active:translate-y-0.5 active:shadow-[inset_0_3px_6px_rgba(0,0,0,0.25)] backdrop-blur-lg transition-all duration-100 ${isActive ? 'bg-gradient-to-b from-cyan-400/80 via-cyan-300/70 to-cyan-200/60 border-cyan-400 shadow-[0_0_20px_rgba(34,211,238,0.6)]' : 'bg-gradient-to-b from-white/30 via-white/20 to-white/15 border-white/40 shadow-[0_2px_8px_rgba(255,255,255,0.15),inset_0_1px_0_rgba(255,255,255,0.4)]'} ${isExpanded ? 'w-10 h-40' : 'w-8 h-24'}`
              }`}
      ...
      {isExpanded && (
        <div className="flex flex-wrap justify-center gap-2 md:gap-3 pt-4 px-2">
          <button onClick={() => setRecording(!recording)} className={`group relative px-4 md:px-6 py-2 md:py-3 rounded-full font-bold text-xs md:text-sm uppercase tracking-wider transition-all ${recording ? 'bg-red-500/20 border-2 border-red-500 text-red-400 shadow-lg shadow-red-500/30 animate-pulse' : 'bg-white/5 border-2 border-white/20 text-zinc-300 hover:border-cyan-500/60 hover:bg-white/10'}`}>
            <span className="relative z-10">{recording ? '⏺ Rec' : '⏺ Record'}</span>
          </button>
          <button onClick={playRecording} disabled={!recordedNotes.length || isPlaying} className={`group relative px-4 md:px-6 py-2 md:py-3 rounded-full font-bold text-xs md:text-sm uppercase tracking-wider transition-all ${isPlaying ? 'bg-green-500/20 border-2 border-green-500 text-green-400 shadow-lg shadow-green-500/30' : recordedNotes.length > 0 ? 'bg-white/5 border-2 border-white/20 text-zinc-300 hover:border-cyan-500/60 hover:bg-white/10' : 'bg-white/5 border-2 border-white/10 text-zinc-600 cursor-not-allowed'}`}>
            <span className="relative z-10">▶ Play ({recordedNotes.length})</span>
          </button>
          <button onClick={() => { setRecordedNotes([]); setIsPlaying(false); }} disabled={!recordedNotes.length} className={`group relative px-4 md:px-6 py-2 md:py-3 rounded-full font-bold text-xs md:text-sm uppercase tracking-wider transition-all ${recordedNotes.length > 0 ? 'bg-white/5 border-2 border-white/20 text-zinc-300 hover:border-red-500/60 hover:bg-red-900/20' : 'bg-white/5 border-2 border-white/10 text-zinc-600 cursor-not-allowed'}`}>
            <span className="relative z-10">⏹ Clear</span>
          </button>
          <div className="flex items-center gap-2 bg-white/5 border-2 border-white/20 rounded-full px-3 md:px-4 py-2 md:py-3">
            <button onClick={() => setOctaveShift(Math.max(-2, octaveShift - 1))} className="text-zinc-300 hover:text-cyan-400 font-bold transition-colors text-lg">−</button>
            <span className="text-xs md:text-sm font-bold text-zinc-400 min-w-[60px] md:min-w-[70px] text-center">Oct {octaveShift > 0 ? '+' : ''}{octaveShift}</span>
            <button onClick={() => setOctaveShift(Math.min(2, octaveShift + 1))} className="text-zinc-300 hover:text-cyan-400 font-bold transition-colors text-lg">+</button>
          </div>
        </div>
      )}
    </div>
...
const GeometricVisualizer = ({ noteTrigger }) => {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      const coreMesh = new THREE.Mesh(new THREE.IcosahedronGeometry(1.2, 1), new THREE.MeshBasicMaterial({ color: 0x22d3ee, wireframe: true, transparent: true, opacity: 0.8 }));
      scene.add(coreMesh);
      const outerMesh = new THREE.Mesh(new THREE.IcosahedronGeometry(2.5, 1), new THREE.MeshBasicMaterial({ color: 0x6366f1, wireframe: true, transparent: true, opacity: 0.15 }));
      scene.add(outerMesh);
      const particlesCount = 1000;
      const posArray = new Float32Array(particlesCount * 3);
      const velocityArray = new Float32Array(particlesCount * 3);
      for (let i = 0; i < particlesCount * 3; i++) { posArray[i] = (Math.random() - 0.5) * 20; velocityArray[i] = (Math.random() - 0.5) * 0.02; }
      const particlesGeo = new THREE.BufferGeometry();
      particlesGeo.setAttribute('position', new THREE.BufferAttribute(posArray, 3));
      particlesGeo.setAttribute('velocity', new THREE.BufferAttribute(velocityArray, 3));
      const particlesMesh = new THREE.Points(particlesGeo, new THREE.PointsMaterial({ size: 0.03, color: 0xffffff, transparent: true, opacity: 0.6, blending: THREE.AdditiveBlending }));
      scene.add(particlesMesh);
      let time = 0;
      const animate = () => {
        requestAnimationFrame(animate); time += 0.01;
        coreMesh.rotation.x += 0.005; coreMesh.rotation.y += 0.008;
        const cs = 1 + Math.sin(time * 2) * 0.05; coreMesh.scale.set(cs, cs, cs);
        outerMesh.rotation.x -= 0.002; outerMesh.rotation.y -= 0.003;
        const os = 1 + Math.sin(time * 1.5) * 0.03; outerMesh.scale.set(os, os, os);
        const p = particlesMesh.geometry.attributes.position.array, v = particlesMesh.geometry.attributes.velocity.array;
        ...
        camera.position.x = Math.sin(time * 0.3) * 0.3; camera.position.y = Math.cos(time * 0.2) * 0.3;
      ...
      return () => { window.removeEventListener('resize', handleResize); if (mountRef.current && renderer.domElement.parentNode) mountRef.current.removeChild(renderer.domElement); };
    };
    document.body.appendChild(script);
...
const SuhasWebsite = () => {
  ...
  // FEATURE 2: scroll progress for the line
  const [scrollProgress, setScrollProgress] = useState(0);

  // FEATURE 7: active section for nav underline
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // FEATURE 2: calculate scroll progress
  ...
  // FEATURE 7: intersection observer for active section
  useEffect(() => {
    if (!loaded) return;
  ...
  return (
    <>
      <FontLoader />
      {/* FEATURE 1: Loading screen — shown before main content */}
        ...
        <div className="min-h-screen bg-black text-white font-sans selection:bg-cyan-500 selection:text-black overflow-x-hidden">
          ...
          <nav className={`fixed w-full z-50 transition-all duration-500 ${scrolled ? 'bg-black/80 backdrop-blur-md py-4' : 'bg-transparent py-6'}`}>
            <div className="container mx-auto px-6 flex justify-between items-center">
              ...
              {/* FEATURE 7: active-section underline on nav */}
              <div className="hidden md:flex space-x-12">
                {navLinks.map((link) => (
                  <a
                    key={link.name}
                    href={link.href}
                    className={`text-sm uppercase tracking-widest transition-colors relative group font-bold ${activeSection === link.href.slice(1) ? 'text-cyan-400' : 'text-white hover:text-cyan-400'}`}
                  >
                    {link.name}
                    <span className={`absolute -bottom-1 left-0 h-0.5 bg-cyan-500 transition-all duration-400 ${activeSection === link.href.slice(1) ? 'w-full' : 'w-0 group-hover:w-full'}`} />
                  </a>
                ))}
              </div>
              ...
              <button
                ...
                className="md:hidden text-white hover:text-cyan-400 z-50 relative"
              >
                <Menu size={28} />
              </button>
            </div>
                  ...
                  <div className="flex flex-col gap-4 items-center w-full">
                    {!showVisualizer ? (
                      <div className="flex gap-4 items-center flex-wrap justify-center">
                        <a href={appleMusicLink} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 hover:border-white/40 flex items-center justify-center transition-all duration-300 hover:scale-110"><Music size={20} className="text-white" /></a>
                        <a href={youtubeLink} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 hover:border-white/40 flex items-center justify-center transition-all duration-300 hover:scale-110"><Youtube size={20} className="text-white" /></a>
          ...
          {!showVisualizer && (
            <div className="w-full relative z-20 -mt-16">
              ...
              <div
                ...
                style={{ background: 'linear-gradient(to right, #360225, #2f43a983, #5f188291)', backgroundSize: '200% 100%', animation: 'gradient-shift 15s ease infinite' }}
              >
                <div className="inline-block" style={{ animation: 'marquee 30s linear infinite' }}>
                  {[...Array(10)].map((_, i) => (
          ...
          {/* ABOUT — FEATURE 3: Syne font for "About" heading */}
          <section id="about" className="min-h-screen flex items-center justify-center py-32 relative overflow-hidden bg-black">
            <div className="absolute inset-0 z-0">
              <img src="/images/suhas.jpeg" alt="Suhas Background" className="w-full h-full object-cover opacity-90 object-center" />
            ...
            <div className="container mx-auto px-6 relative z-10 grid grid-cols-1 md:grid-cols-2 gap-16 items-center max-w-7xl">
              <RevealOnScroll>
                <div className="space-y-8">
                  {/* FEATURE 3: Syne font on About heading */}
                  <h2
                ...
                <div className="lg:w-1/2 space-y-10">
                  ...
                  <RevealOnScroll delay={200}>
                    <div className="flex flex-wrap gap-4">
                      <a href={appleMusicLink} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-2 px-8 py-4 border border-zinc-800 hover:border-cyan-500 hover:bg-zinc-900 transition-all uppercase text-sm font-bold tracking-wider rounded-full">Apple Music <ExternalLink size={14} className="group-hover:-translate-y-1 group-hover:translate-x-1 transition-transform" /></a>
                      <a href={spotifyLink} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-2 px-8 py-4 border border-zinc-800 hover:border-cyan-500 hover:bg-zinc-900 transition-all uppercase text-sm font-bold tracking-wider rounded-full">Spotify <ExternalLink size={14} className="group-hover:-translate-y-1 group-hover:translate-x-1 transition-transform" /></a>
                      <a href={youtubeLink} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-2 px-8 py-4 border border-zinc-800 hover:border-cyan-500 hover:bg-zinc-900 transition-all uppercase text-sm font-bold tracking-wider rounded-full">Youtube <ExternalLink size={14} className="group-hover:-translate-y-1 group-hover:translate-x-1 transition-transform" /></a>
                    </div>
                  </RevealOnScroll>
                </div>
              </div>
          ...
          {/* CONTRIBUTE — FEATURE 4: new fonts from experiment branch */}
            ...
            <div className="container mx-auto px-6 max-w-6xl relative z-10">
              <RevealOnScroll>
                <div className="mb-16 text-center">
                  <h2
                    className="text-3xl sm:text-4xl md:text-6xl uppercase tracking-tight mb-2"
                    style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800 }}
                  >
                    Contribute
                  </h2>
                  <div className="w-20 h-1 bg-cyan-500 mx-auto" />
                  <p className="text-zinc-300 text-lg sm:text-xl md:text-2xl max-w-3xl mx-auto mt-8 font-light leading-relaxed">
                    Help bring the next studio album to life
                  </p>
                  <p className="text-cyan-400 text-base sm:text-lg font-medium mt-4">
                    ...
                    <div className={`group relative bg-gradient-to-br from-zinc-900 to-zinc-950 border-2 border-zinc-800 p-6 sm:p-10 flex flex-col items-center gap-5 sm:gap-6 transition-all duration-500 rounded-2xl cursor-default overflow-hidden ${c.color === 'green' ? 'hover:border-green-500 hover:shadow-2xl hover:shadow-green-500/20' : 'hover:border-yellow-500 hover:shadow-2xl hover:shadow-yellow-500/20'}`}>
                      ...
                      <div className="relative z-10 w-full flex flex-col items-center gap-5 sm:gap-6 flex-1">
                        ...
                        <div className="text-center">
                          <h3
                            className="text-2xl sm:text-3xl uppercase mb-2 sm:mb-3 text-white"
                            style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800 }}
                          >
                            {c.title}
                          </h3>
                        ...
                        <div className="mt-auto pt-3 sm:pt-4">
                          <span className={`inline-block text-sm sm:text-lg font-bold uppercase tracking-widest ${c.color === 'green' ? 'text-green-500' : 'text-yellow-500'} border-b-2 border-transparent group-hover:border-current transition-all pb-1`}>
                            Coming Soon
                          </span>
                        </div>
                      </div>
          ...
          {/* STORE — FEATURE 5: Syne font on Store heading */}
            ...
            <div className="container mx-auto px-6">
              <RevealOnScroll>
                <div className="flex justify-between items-end mb-16">
                  <div>
                    <h2
                      className="text-4xl md:text-6xl uppercase tracking-tight mb-2"
                      style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800 }}
                    >
                      Store
                    </h2>
                  ...
                  <span className="hidden md:flex items-center gap-2 text-zinc-400 cursor-default text-sm uppercase tracking-widest">View All <ArrowRight size={16} /></span>
                </div>
              </RevealOnScroll>
                    ...
                    <RevealOnScroll key={item.id} delay={i * 150}>
                      <Tag {...props} className={`group block ${item.link ? 'cursor-pointer' : 'cursor-default'}`}>
                        <div className="aspect-square bg-zinc-900 mb-6 relative overflow-hidden border border-zinc-800">
                          {imgMap[item.type] && (
                            <img src={imgMap[item.type]} alt={item.name} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                          )}
                          {item.link && (
                            <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-sm">
                              <span className="text-white font-bold uppercase tracking-widest border border-white px-6 py-3 hover:bg-white hover:text-black transition-colors text-xs">Buy Now</span>
                            </div>
                          )}
                        </div>
                        <div className="flex justify-between items-start">
                          <div>
                            <h3 className="text-xl font-bold uppercase mb-1" style={{ fontFamily: "'Syne', sans-serif" }}>{item.name}</h3>
          ...
          {/* CONNECT — FEATURE 6: new connect section from experiment branch */}
          <section id="connect" className="min-h-screen flex items-center py-28 relative overflow-hidden bg-[#030306] border-t border-zinc-900">
            ...
            <div className="container mx-auto px-6 max-w-5xl relative z-10">
              <div className="text-center mb-14">
                ...
                <RevealOnScroll delay={100}>
                  <h2
                    className="text-[clamp(2.5rem,8vw,5.5rem)] tracking-[-0.04em] leading-[0.9] mb-4"
                    style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800 }}
                  >
                    Connect
                  </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
                {[
                  { href: instagramLink, icon: <Instagram size={26} />, name: 'Instagram', handle: '@suhas.als', hoverBorder: 'hover:border-purple-500/50', hoverShadow: 'hover:shadow-purple-500/20', hoverBg: 'group-hover:from-purple-500/5 group-hover:via-pink-500/8 group-hover:to-orange-500/5', iconHover: 'group-hover:shadow-purple-500/30', blob: 'group-hover:bg-purple-500/10' },
                  { href: youtubeLink, icon: <Youtube size={26} />, name: 'YouTube', handle: '@suhasmusicofficial', hoverBorder: 'hover:border-red-500/50', hoverShadow: 'hover:shadow-red-500/20', hoverBg: 'group-hover:from-red-500/5 group-hover:to-red-500/10', iconHover: 'group-hover:shadow-red-500/30', blob: 'group-hover:bg-red-500/10' },
                  { href: appleMusicLink, icon: <Headphones size={26} />, name: 'Music', handle: 'Stream now', hoverBorder: 'hover:border-rose-500/50', hoverShadow: 'hover:shadow-rose-500/20', hoverBg: 'group-hover:from-rose-500/5 group-hover:to-fuchsia-600/8', iconHover: 'group-hover:shadow-rose-500/30', blob: 'group-hover:bg-rose-500/10' },
                ].map((s, i) => (
                  <RevealOnScroll key={s.name} delay={i * 80}>
                    ...
                    >
                      ...
                      <div className="text-center relative">
                        <p className="text-lg font-bold mb-1" style={{ fontFamily: "'Syne', sans-serif" }}>{s.name}</p>
            ...
            <div className="container mx-auto px-6">
              <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-8">
                ...
                <div className="flex gap-4">
                  <a href={instagramLink} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-400 hover:text-cyan-400 hover:border-cyan-500 hover:bg-zinc-800 transition-all" aria-label="Instagram"><Instagram size={18} /></a>
                  <a href={youtubeCreatorLink} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-400 hover:text-cyan-400 hover:border-cyan-500 hover:bg-zinc-800 transition-all" aria-label="YouTube"><Youtube size={18} /></a>
                  <a href={appleMusicArtistLink} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-400 hover:text-cyan-400 hover:border-cyan-500 hover:bg-zinc-800 transition-all" aria-label="Apple Music"><Music size={18} /></a>
